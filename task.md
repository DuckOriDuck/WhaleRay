...
- [x] **Step 2.1: 코드 베이스 동기화 (Baseline)**
    - [x] `git pull origin main`으로 최신 코드를 가져온다. (이것이 기준점이 됨)
- [x] **Step 2.2: 함수 복제 (Duplicate)**
    - [x] `lambda/repo_inspector` 폴더 전체를 `lambda/env_builder`로 복사한다. (의존성 및 설정 보존 목적)
- [x] **Step 2.3: 가지치기 (Prune & Connect)**
# Google Antigravity Task Guidelines
...
- [x] **Step 2.2: 함수 복제 (Duplicate)**
    - [x] `lambda/repo_inspector` 폴더 전체를 `lambda/env_builder`로 복사한다. (의존성 및 설정 보존 목적)
- [x] **Step 2.3: 가지치기 (Prune & Connect)**
    - [x] `repo_inspector`: **SSM/CodeBuild 로직만 삭제**하고, 끝부분에 `invoke` 코드를 심는다. (나머지 로직 건드리지 않음)
    - [x] `env_builder`: **GitHub/분석 로직만 삭제**하고, `event`에서 데이터를 꺼내도록 진입점을 수정한다.
- [x] **Step 2.4: 인프라 수정 (Terraform)**
    - [x] `lambda.tf`: `env_builder` 리소스 정의.
    - [x] `iam.tf`: 권한을 기능에 맞게 정확히 분리(Split).

### 🚧 Phase 3: 통합 및 배포 [진행중]
- [x] **Step 3.0: 인프라 안정화 (Infrastructure Fixes)**
    - [x] `dynamodb.tf`: `deployments` 테이블 Stream 활성화 (`stream_enabled = true`).
    - [x] `lambda.tf`: `repo_inspector` 트리거를 위한 DynamoDB Stream IAM 권한 추가.
    - [x] `create_zip.py`: `source_code_hash` 불일치 해결을 위한 Zip 생성 결정론적(Deterministic) 수정.
    - [x] **검증 완료:** 로컬 테스트 결과 Zip 파일 해시 일치 확인.
- [x] **Step 3.1: 타겟 배포 (Target Apply)**
    - [x] 영향 범위를 최소화하기 위해 `-target` 옵션 사용.
- [ ] **Step 3.2: 최종 E2E 테스트**