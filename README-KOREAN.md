...
각 서비스는 고유한 서브도메인과 경로를 받으며, 배포 중에 무중단을 보장하는 원활한 버전 업데이트를 제공합니다.
...
동일한 브랜치에서 배포하면, 기존 서비스가 새 빌드 결과로 원활하게 교체됩니다.

### 동적 트래픽 라우팅

- **Nginx 라우터**: 정규 표현식 패턴(`^/(?<deployment_id>github_[^/]+)`)을 사용하여 대상 서비스를 식별하고 그에 따라 트래픽을 라우팅합니다
- **서비스 디스커버리(AWS Cloud Map)**: ECS 태스크가 자동으로 `whaleray.local` 네임스페이스에 프라이빗 IP를 등록하여, Nginx가 변경되는 컨테이너 IP를 동적으로 발견할 수 있습니다

### 배포 전략

- ECS 서비스는 **최소 정상 비율: 100%** 및 **최대 비율: 200%**로 구성됩니다
- 배포 중에 새 컨테이너가 100% 정상(실행 중) 상태에 도달한 후 기존 컨테이너가 드레이닝 및 종료되기 시작합니다
- 이 전략은 무중단 업데이트를 보장하여 지속적인 서비스 가용성을 유지합니다
...
- 추출된 로그는 DevOps 엔지니어 페르소나가 주입된 프롬프트와 함께 Claude 3로 전송됩니다
- 평문 대신 구조화된 JSON 출력(`status`, `issues`, `recommendations`)을 반환하여 프론트엔드에서 즉시 시각화할 수 있습니다
